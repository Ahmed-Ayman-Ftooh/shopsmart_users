...
class ProductProviders with ChangeNotifier {
  ...
  List<ProductModels> get getProducts {
    return _products;
  }

  ProductModels? findByProdId(String productId) {
  ...
  List<ProductModels> findByCategory({required String ctgName}) {
    List<ProductModels> ctgList =
        _products
            .where(
  ...
  List<ProductModels> searchQuery({
    required String searchText,
    required List<ProductModels> passedList,
  }) {
    ...
    try {
      await productDB.orderBy('createdAt', descending: false).get().then((
        ...
        for (var element in productsSnapshot.docs) {
          _products.insert(0, ProductModels.fromFirestore(element));
        }
      });
      notifyListeners();
      return _products;
  ...
  Stream<List<ProductModels>> fetchProductsStream() {
    try {
